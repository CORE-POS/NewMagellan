...
namespace SPH {
    ...
    /// <summary>
    /// This serial handler is a legacy implementation that
    /// has been replaced by SPH_Magellan_Scale. The primary
    /// difference is in how the two implementations monitor
    /// the scale portion of the scanner scale.
    /// 
    /// SPH_Magellan_Scale *only* uses the "get current status" 
    /// command to check on the scale. This command is repeated
    /// infinitely.
    /// 
    /// SPH_Magellan_Classic uses a mix of the "get current status"
    /// command and the "get next non-zero stable weight" command
    /// depending on the current state of the scale. The logic is
    /// based on the original VB implementation. Using this approach
    /// seems to cause more "beep of death" issues where the scale
    /// beeps loudly then stops weighing until rebooted. This is
    /// particularly frequent with Omega edition scales.
    /// 
    /// Why the problem exists is still unknown. My guess is that
    /// if the scale is sitting in a below-zero state for too long
    /// after the "get next non-zero stable weight" command some
    /// timeout triggers an error condition and the scale portion
    /// of the scanner-scale shuts down.
    /// </summary>
    public class SPH_Magellan_Classic : SerialPortHandler {
